...
class TimeSeriesDatasetLazy(Dataset):
    def __init__(
        self, data_root, tasks, split="TEST", sequence_length=512, balance=False
    ):
        # === Balanced ===
        if balance:
            dataset_dirs = []
            ...
            if split == "TRAIN":
                ...
                for dataset in files:
                    if len(dataset) < max_dataset_samples:
                        data.extend(
                            dataset
                            * np.ceil(max_dataset_samples / len(dataset)).astype(int)
                        )
                    else:
                        data.extend(dataset)
            else:
    ...
    def __getitem__(self, idx):
        x = (
            torch.from_numpy(pd.read_csv(self.data[idx]).values.flatten())
            .float()
            .view(1, -1)
        )

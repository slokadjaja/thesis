...
from benchmarks.VQShape.data_provider.timeseries_loader import TimeSeriesDatasetLazy
from benchmarks.VQShape.vqshape.vqshape_utils import visualize_shapes, plot_code_heatmap
from benchmarks.VQShape.vqshape.model import VQShape
...
class LitVQShape(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (
            self.hparams.lambda_x * loss_dict["ts_loss"].mean()
        ...
        loss_dict = {
            "TRAIN/total": loss,
            "TRAIN/x": loss_dict["ts_loss"].mean(),
            "TRAIN/z": loss_dict["vq_loss"].mean(),
            "TRAIN/s": loss_dict["shape_loss"].mean(),
            "TRAIN/dist": loss_dict["dist_loss"].mean(),
        }
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.validation_step_outputs[dataloader_idx]["z"].append(
            output_dict["code_idx"]
        )
    ...
    def on_validation_epoch_end(self):
        for i in self.validation_step_outputs.keys():
            ...
            fig = plot_code_heatmap(
                z_idx, self.hparams.num_code, title=f"{self.global_step}"
            )
            self.logger.experiment.log(
                {
    ...
    def configure_optimizers(self):
        ...
        stepping_batches = (
            self.trainer.estimated_stepping_batches - self.hparams.warmup_step
        )
        ...
        scheduler = torch.optim.lr_scheduler.SequentialLR(
            ...
            schedulers=[
                torch.optim.lr_scheduler.LinearLR(
                    optimizer, 0.0001, 1, self.hparams.warmup_step
                ),
                torch.optim.lr_scheduler.CosineAnnealingLR(
                    optimizer, T_max=stepping_batches, eta_min=1e-5
                ),
            ],
...
def get_args():
    ...
    parser.add_argument(
        "--codebook_type", type=str, default="standard", choices=["standard", "vqtorch"]
    )
    ...
    parser.add_argument("--lambda_vq_commit", type=float, default=1.0)
    parser.add_argument("--lambda_vq_entropy", type=float, default=1.0)
    parser.add_argument("--entropy_gamma", type=float, default=1.0)

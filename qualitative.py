...
def plot_decoded_symbols(model: VAE, params: Params, plots_dir: Path):
    ...
    for pos in range(params.n_latent):  # iterate over positions in encoding
        ...
        ):  # iterate over possible values for position
...
def plot_patch_groups(
    ...
    if model_type == "vae":
        ...
        split_segments = [
            np.array_split(row, model_spec["n_segments"]) for row in X_train
        ]
        ...
        if plot_individual:
            ...
            if model_type == "vae":
                ...
                padded_segments = np.array(
                    [
                        np.pad(patch, (0, max_len - len(patch)), constant_values=np.nan)
                        for patch in patches_group
                    ]
                )
...
def plot_symbol_distributions(
    ...
    if group_by_class:
        ...
        for cls in unique_classes:
            ...
            _plot_distribution(
                class_encodings, f"symbol_dist_cls{cls}_{vis_method}.png"
            )
    else:
...
def intra_class_similarity(x):
    ...
    dist_arr = [
        1 - distance.hamming(enc1, enc2) for enc1, enc2 in itertools.combinations(x, 2)
    ]
...
def compare_all_seq(x, y):
    ...
    class_sequences = {
        label: {tuple(seq) for seq in x[y == label].tolist()} for label in unique_labels
    }  # Group by class
...
def compare_ngram_sets(x, y, n=3, min_support=0.8):
    ...
    unique_labels = np.unique(y)  # Get unique class labels
    class_sequences = {
        label: x[y == label] for label in unique_labels
    }  # Group by class
    ...
    for label, ngram_set in class_ngrams.items():
        unique_ngrams[label] = ngram_set - set.union(
            *(class_ngrams[l] for l in class_ngrams if l != label)
        )
...
def plot_jaccard_heatmap(
    jaccard_scores: dict[tuple, float], labels: list, plots_dir: Path
):
    """
    ...
    sns.heatmap(
        jaccard_matrix, annot=True, cmap="Blues", xticklabels=labels, yticklabels=labels
    )
...
def get_common_subsequences(X_encoded, y, plots_dir):
    ...
    for c in classes:
        ...
        result = PrefixSpan(filtered_time_series).frequent(
            math.floor(min_support * len(filtered_time_series)), closed=True
        )
        result = [
            r for r in result if len(r[1]) >= 3
        ]  # pick sequences with 3 elements or longer
        print(f"class: {c}")
        print(result)
